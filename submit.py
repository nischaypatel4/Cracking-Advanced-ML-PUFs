...
def construct_phi_11_base(X):

    X = np.asarray(X)
    # map {0,1}→{+1,−1}
    d = 1 - 2*X
    n = X.shape[0]
    phi = np.empty((n, 11), dtype=d.dtype)
    # 0) bias
    phi[:, 0] = 1
    # 1–8) raw bits
    phi[:, 1:9] = d
    # 9) full‑product of bits 0..7
    phi[:, 9]  = np.prod(d, axis=1)
    # 10) last‑2‑suffix: product of bits 6 and 7
    phi[:, 10] = d[:, 6] * d[:, 7]
    return phi  # shape (n,11)
...
################################
	...
	Z = my_map(X_train)
    # print(f"Map: {map_func.__name__}, feature‑dim = {Z.shape[1]}")
	scaler = StandardScaler().fit(Z)
	Zs = scaler.transform(Z)
	clf = LogisticRegression(C=100, solver='liblinear', max_iter=500, random_state=42)
	clf.fit(Zs, y_train)
    # return clf, scaler

    # X_train_mapped = my_map(X_train)
    # model = LinearSVC(C=50, max_iter=1000, dual = False)
    # # model = LogisticRegression(penalty='l2', solver='lbfgs', max_iter=5000, fit_intercept=False, C = 10.0)
...
################################
    ...
    phi = construct_phi_11_base(X)
    n, d = phi.shape               # d = 11
    i, j = np.triu_indices(d)      # indices of upper triangle
    # compute all outer products in one go
    outs = np.einsum('bi,bj->bij', phi, phi)  # (n,11,11)
    # extract only the upper‑triangle entries
    feats = outs[:, i, j]                     # (n,66)
    return feats
